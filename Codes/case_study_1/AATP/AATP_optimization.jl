# @info "Entering Optimization Module"
...
############################# Assigning penalties to each order #############################
γ = zeros(maximum(K)) # denial penalty
γp = zeros(maximum(K)) # partial rejection penalty
for k in K
       γ[k] = πS[O[k]]*prt[k]
       γp[k] = γ[k]*0.75
end
...
############################# Fixed decisions on orders from previous run #############################
for k in eachindex(decision)
       if(decision[k]>-0.5) # fixed decisions for accepted orders 
              ...
              if(due_time[k]>1 && OiD[k] in order_Id)
...
############################# state-space framework - using lifting variables to track information #############################

############################# Task Unit asssignment constraints #############################
if(iter>1)
...
if (iter>1)
       ...
       println(start_time, Fixed_Horizon, Batching_Interval+1)
else
...
############################# lower and upper bound on order quanitites #############################

for k in K
       if (OiD[k] in order_Id)
              @constraint(STNSch,[t = de[k]:dl[k]], C[k,t] <= cl[k]*D[k,t]) 
...
# for k in K
#        println("order id ", OiD[k], " decision ", value(Z[k]), " quantity ", value(sum(C[k,t] for t in de[k]:dl[k])), " due date ", value(sum(D[k,t]*t for t in de[k]:dl[k])))
# end
...
############################# Storing information for ongoing batches to be used in next execution #############################
for i in I
    for j in J
