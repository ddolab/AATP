...
println("Initial inventory: ", INV_initial)
...
# end
       ...
       # println(B_SS_initial[1,1,start_time:start_time+25,1])
...
for k in K
       if (OiD[k] in order_Id)
              ...
              @constraint(STNSch,[t = de[k]:dl[k]], C[k,t] <= 0*round(ce[k],digits=3)*D[k,t]) 
       end
end
...
println("New initial inventory at ", start_time+Batching_Interval, " =  ", INV_initial)
