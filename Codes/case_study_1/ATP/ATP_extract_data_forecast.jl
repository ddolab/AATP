...
# println(T)
XLSX.openxlsx("ATP-Data-output-BI-$Batching_Interval-scenario-$scenario-forecast-schedule_iteration-$schedule_iter.xlsx", mode="w") do xf
       sheet = xf[1]
       sheet["A1"] = "Time"
       sheet["B1"] = "Task/Units"
       row = 1
       ...
       for t in T
              ...
              column += 1
              for s in S
                     sheet1[row,column] = value(Inv[s, t]) + sum(value(C[k, F_de[k]]) for k in K if F_OiD[k] > 1000 && F_O[k] == s && F_de[k] <= t; init=0.0)
