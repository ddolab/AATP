...
for k in K
       if (OiD[k] in order_Id)
              ...
              @constraint(STNSch,[t = de[k]:dl[k]], C[k,t] <= trunc(ce[k],digits=7)*D[k,t])
       end
end
...
if (start_time+Batching_Interval-1<scheduling_end_time)
       INV_initial[:] = trunc.(value.(Inv[:,start_time+Batching_Interval]),digits=7)
    ...
    for j in J
       for t in eachindex(T)
              for m in 1:τ[i]+1
                     ...
                     if (value.(B_SS[i,j,T[t],m])  > 0.0001)
                            B_SS_initial[i,j,t,m] = trunc.(value.(B_SS[i,j,T[t],m]),digits=7)
