...
FQ = trunc.(sum(FQ_Demand)./FQ_Demand_scenario,digits=7)
...
for i in eachindex(order_Id)
    ...
    if(Product_Type[i]==6)
        ...
        FQ[idx+42] += Min_Order_Quantity[i]*rand(80:120)/100   
    end
end
...
for itr = 1:last_itr
    ...
    #     forecast_demand_6 = trunc((total_forecast_demand_6 - total_demand_6)/14,digits=7)
    # end
    # if (total_demand_9 < total_forecast_demand_9)
    #     forecast_demand_9 = trunc((total_forecast_demand_9 - total_demand_9)/14,digits=7)
    # end
            ...
            for net_idx in netted_indicies
                if (FQ_ID[net_idx] in OiD && FQ_P[net_idx] == Product_Type[idx] && FQ[net_idx] > 0)
                    ...
                    if (FQ[net_idx] >= netted_qty)
                        ...
                        # println("after ",ce)
                        FQ[net_idx] = trunc(FQ[net_idx],digits=7)
                        ce[index] = trunc(ce[index],digits=7)
    ...
    for k in eachindex(OiD)
            if(al[k] <= start_time)
                ...
                accepted_quantities[k] = min(cl[k],trunc(sum(value(C[k,t]) for t in de[k]:dl[k]),digits=7))
                # accepted_quantities[k] = sum(value(C[k,t]) for t in de[k]:dl[k])
            end
            if(OiD[k] in order_Id)
                global idx = findfirst(id->id==OiD[k],order_Id)
                order_decision[idx] = value(Z[k]) > 0.5 ? 1 : 0
                Order_promised_quantites[idx] = min(cl[k],trunc(sum(value(C[k,t]) for t in de[k]:dl[k]),digits=7))
                ...
                if (value(Z[k]) < 0.5)
                    println("rejeted order id: ",OiD[k])
                end
            end      
    ...
    for k in indicies
        ...
        if(OiD[k] in FQ_ID)
            ...
            INV_initial[O[k]] += trunc(sum(value(C[k,t]) for t in de[k]:dl[k]),digits=7)
        end

        if(OiD[k] in order_Id)
            global idx = findfirst(id->id==OiD[k],order_Id)
            order_decision[idx] =  value(Z[k]) > 0.5 ? 1 : 0
            Order_promised_quantites[idx] = trunc(sum(value(C[k,t]) for t in de[k]:dl[k]),digits=7)
            # Order_promised_quantites[idx] = sum(value(C[k,t]) for t in de[k]:dl[k])
            order_assigned_due_time[idx] = Int(sum((value(D[k,t]) > 0.5 ? 1 : 0 )*t for t in de[k]:dl[k]))
        end
    end
    ...
    for k in eachindex(OiD)
        if(al[k] <= start_time)
            ...
            if (order_decision[idx]<0.5)
                push!(indicies,k)
                println("Removing - Rejected oders and response time over: ",OiD[k], " actual ", order_Id[idx])
            end
        end
    end
    deleteat!(OiD,indicies)
