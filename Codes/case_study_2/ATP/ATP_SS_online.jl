...
for s in S
       println("Initial Inventory s ", s , " =  ", INV_initial[s])
end

println("Total orders ",length(OiD))
...
# # Order arrival/customer response time    
# ae = [1,3,1,2]
# al = [4,8,9,10]
# OiD = [44,55,56,33]
# # earliest / latest due time
# de = [3,5,5,9]
# dl = [5,18,12,24]
 
# # Order minimum quanitites
# ce = [52,40,50,80]
# # cl = [10 50 80]
# cl = ce
...
# set_optimizer_attribute(STNSch,"FeasibilityTol",1e-9)
...
Z[K], Bin       # If a order k is accepted or nots
C[k in K, t in de[k]:dl[k]] >= 0     # quantity of order k promised in time t
D[k in K, t in de[k]:dl[k]], Bin     # delivery timeframe indicator for order k
...
for k in eachindex(decision)
       if(decision[k]>-0.5 && OiD[k] in order_Id) # fixed decisions for accepted orders 
              @constraint(STNSch,Z[k]==decision[k])
              # println("Fixing decision for order " , OiD[k])
              if(due_time[k]>1)
                     # println("Fixing the due date and quanitites ",OiD[k]," ", due_time[k]," ",accepted_quantities[k])
                     @constraint(STNSch,D[k,due_time[k]]==1)
                     # @constraint(STNSch,C[k,due_time[k]]==min(cl[k],round(accepted_quantities[k],digits=3)))
                     @constraint(STNSch,C[k,due_time[k]]==min(cl[k],accepted_quantities[k]))
              end
       end
end
...
#        for i in I, j in J, t in Fixed_Horizon+1:T[end]-Batching_Interval, m in 1:τ[i]+1
#               # Set the start value only for specific variables
#               set_start_value(X_SS[i, j, t, m], X_SS_initial[i, j, t - start_time + Batching_Interval + 1, m])
#               set_start_value(B_SS[i, j, t, m], B_SS_initial[i, j, t - start_time + Batching_Interval + 1, m])
#        end

# end

@constraint(STNSch, [i in I, j in J, t in start_time:T[end], m=1:τ[i]+1],X_SS[i,j,t,m] == X_SS_initial[i,j,t,m]) 
@constraint(STNSch, [i in I, j in J, t in start_time:T[end], m=1:τ[i]+1],B_SS[i,j,t,m] == B_SS_initial[i,j,t,m]) 

# if (iter>1)
#        @constraint(STNSch, [i in I, j in J, t in start_time:Fixed_Horizon, m=1:τ[i]+1],X_SS[i,j,t,m] == X_SS_initial[i,j,t-start_time+Batching_Interval+1,m]) 
#        @constraint(STNSch, [i in I, j in J, t in start_time:Fixed_Horizon, m=1:τ[i]+1],B_SS[i,j,t,m] == B_SS_initial[i,j,t-start_time+Batching_Interval+1,m]) 
# else
...
for k in K
       if (OiD[k] in order_Id)
              @constraint(STNSch,[t = de[k]:dl[k]], C[k,t] <= cl[k]*D[k,t]) ## ce is maximum order quantites, cl is minimum order quantites
              @constraint(STNSch,[t = de[k]:dl[k]], C[k,t] >= ce[k]*D[k,t])
       else
              @constraint(STNSch,Z[k] == 0)
              # @constraint(STNSch,[t = de[k]:dl[k]], C[k,t] <= round(ce[k],digits=3)*D[k,t])
              @constraint(STNSch,[t = de[k]:dl[k]], C[k,t] <= ce[k]*D[k,t])
       end
end
# @constraint(STNSch,[k in K, t = de[k]:dl[k]], C[k,t] == ce[k]*D[k,t])
# @constraint(STNSch,[k in K, t = de[k]:dl[k]], C[k,t] >= ce[k]*0.5*D[k,t])
# order should be deliverd once Only
@constraint(STNSch,[k in K], sum(D[k,t] for t in de[k]:dl[k]) == Z[k])
# @constraint(STNSch,[k in K], sum(D[k,t] for t in T) <= 1)

# @constraint(STNSch,[k in K], sum(D[k,t] for t in de[k]:dl[k] if (t-1)%24==0) == Z[k])
...
@constraint(STNSch,[t in T, n in N], sum(C[k,t] for k in K if O[k]==n && t >= de[k] && t <= dl[k]) == RMS[n,t])
...
if (iter ==1)
       @constraint(STNSch,[s in S, t=[T[1]]], Inv[s,t]== INV_initial[s]-sum(ρ_in[i,s]*B_SS[i,j,t,1] for i in I for j in J if I_J[j,i]>0.1) - RMS[s,t] +
       sum(ρ_out[i,s]*B_SS[i,j,t,τ_bar[i,s]+1] for i in I for j in J if I_J[j,i]>=0.1) + RMP[s,t])
else
       @constraint(STNSch,[s in S, t=[T[1]]], Inv[s,t]== INV_initial[s]- RMS[s,t] + RMP[s,t]) 
end
...
# No purchase at the end of horizon
# @constraint(STNSch,[s in S,t=T[end]], RMP[s,t]==0)
...
@objective(STNSch,Max,sum(RMS[s,t]*πS[s] for s in S for t in T if !(s in No_sell)) 
                     ...
                     - sum(γ[k]*ce[k]*(1-Z[k]) for k in K)
                     - sum(γp[k]*(Z[k]*cl[k]-sum(C[k,t] for t in de[k]:dl[k])) for k in K)
                     - sum(λ[s]*Inv[s,t] for s in S for t in T)
         )

       
# print(STNSch)
# sum(πF[j]*X_SS[i,j,t,1] + πV[j]*B_SS[i,j,t,1] for i in I for j in J for t in T if I_J[j,i]>0.1)
# @info "Variables, Constraints and the objective has been defined, Executing the model now"
optimize!(STNSch)
...
#     set_optimizer_attribute(STNSch,"IntFeasTol", 1e-4)  
    set_time_limit_sec(STNSch, 3600.0)
    set_optimizer_attribute(STNSch, "MIPGap", 0.01)  # 1% optimality gap
    optimize!(STNSch)
end
...
# Calculate and print the total sum of each term separately


println(solution_summary(STNSch))
for k in K
       print(k, " ", OiD[k], " ", value(Z[k])," ")
       for t in de[k]:dl[k]
              if(value(D[k,t])>0)
                     print(t, " ", value(C[k,t]))
              end
...
if (start_time+Batching_Interval-1<scheduling_end_time)
       # INV_initial[:] = round.(value.(Inv[:,start_time+Batching_Interval]),digits=3)
       INV_initial[:] = value.(Inv[:,start_time+Batching_Interval])
       # INV_initial[:] = value.(Inv[:,start_time+Batching_Interval])
end

# for s in S
#        INV_initial[s] = trunc(INV_initial[s],digits=3)
#        println("Initial Inventory at end ", s , " =  ", INV_initial[s])
# end
...
#               for m in 1:τ[i]+1
#                      X_SS_initial[i,j,t,m] = (value.(X_SS[i,j,T[t],m])) > 0.5 ? 1 : 0
#                      if (value.(B_SS[i,j,T[t],m])  > 0.5)
#                             B_SS_initial[i,j,t,m] = round.(value.(B_SS[i,j,T[t],m]),digits=3)
#                             # B_SS_initial[i,j,t,m] = value.(B_SS[i,j,T[t],m])
#                      else
