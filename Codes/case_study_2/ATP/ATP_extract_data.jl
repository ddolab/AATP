...
XLSX.openxlsx("BI-$Batching_Interval/ATP-Data-output-BI-$Batching_Interval-scenario-$scenario-forecast-iteration-$iter.xlsx", mode="w") do xf
       ...
       for t in T
          ...
          col+=1
          sheet[row,col] =  sum(value(RMS[s,t])*πS[s] for s in S) - sum(value(RMP[s,t])*πP[s] for s in S)  - sum(λ[s] * (value(Inv[s, t]) + sum(value(C[k, de[k]]) for k in K if OiD[k] > 1000 && O[k] == s && de[k] <= t; init=0.0)) for s in S) -  sum(value(UT[l,t])*πU[l] for l in L)
       end
   end
