...
XLSX.openxlsx("Data/ATP_Cost-Data-output-BI-$Batching_Interval-scenario-$scenario-forecast.xlsx", mode="rw") do xf
   ...
   for t in T
      ...
      sheet1[row,col] = t
      col+=1
      sheet[row,col] = sum(value(RMP[s,t])*πP[s] for s in S)
      col+=1
      sheet[row,col] = sum(value(RMS[s,t])*πS[s] for s in S)
      col+=1
      sheet[row,col] = sum(value(UT[l,t])*πU[l] for l in L)
      col+=1
      sheet[row,col] = sum(λ[s] * (value(Inv[s, t]) + sum(value(C[k, de[k]]) for k in K if OiD[k] > 1000 && O[k] == s && de[k] <= t; init=0.0)) for s in S)
      col+=1
      sheet[row,col] =  sum(value(RMS[s,t])*πS[s] for s in S) - sum(value(RMP[s,t])*πP[s] for s in S)  - sum(value(UT[l,t])*πU[l] for l in L) -  sum(λ[s] * (value(Inv[s, t]) + sum(value(C[k, de[k]]) for k in K if OiD[k] > 1000 && O[k] == s && de[k] <= t; init=0.0)) for s in S)
      col+=1
      col=2
      for i in I
   ...
   for t in T
      ...
      for s in S
         ...
         sheet5[row,column] =  sum(ρ_out[i,s]*value(B_SS[i,j,t,τ_bar[i,s]+1]) for i in I for j in J if I_J[j,i]>=0.1)
         sheet6[row,column] = -sum(ρ_in[i,s]*value(B_SS[i,j,t,1]) for i in I for j in J if I_J[j,i]>0.1) 
         column+=1
      end
      column = 2
      for l in L
         sheet4[row,column] = value(UT[l,t])
         column+=1
      end
