...
FQ_Total_orders = 84
FQ_P1_Total_orders = 42
FQ_P2_Total_orders = 42
FQ_ID = (1101:2000)[1:FQ_Total_orders]
FQ_P = vcat(fill(6,FQ_P1_Total_orders),fill(9,FQ_P2_Total_orders))
FQ_Demand = []
FQ_Demand_scenario = 5
for i in 1:FQ_Demand_scenario
    demand = []
    for j in 1:FQ_Total_orders
        # push!(demand, FQ_P[j]==6 ? rand(20:100) : rand(20:150))
        push!(demand, FQ_P[j]==6 ? 150 : 250)
        # push!(demand,100)
    end
    push!(FQ_Demand,demand)
end

FQ = round.(sum(FQ_Demand)./FQ_Demand_scenario,digits=1)
...
Product_Type = shuffle!(vcat(fill(6,P1_Total_orders),fill(9,P2_Total_orders)))
Order_priority_p1 = shuffle!(vcat(fill(2,Int(P1_Total_orders/2)),fill(1,Int(P1_Total_orders/2))))
Order_priority_p2 = shuffle!(vcat(fill(2,Int(P2_Total_orders/2)),fill(1,Int(P2_Total_orders/2))))
...
while order_end_time <= Last_arrival_time + Order_batch_interval
    ...
    for idx in indicies
        ...
        mnq = Product_Type[idx]==6 ? rand(20:100) : rand(20:200)
        push!(Earliest_Due_Time,EDT)
        push!(Latest_Due_Time,LDT)
        push!(Response_Time,RT)
        push!(Min_Order_Quantity,mnq*rand(5:10)/10)
...
for i in eachindex(order_Id)
    ...
    if(Product_Type[i]==6)
        FQ[idx] += trunc(Min_Order_Quantity[i]*rand(80:120)/100,digits=1)
        FQ[idx] = trunc(FQ[idx],digits=1)
    else
        FQ[idx+42] += trunc(Min_Order_Quantity[i]*rand(80:120)/100,digits=1)  
        FQ[idx+42] = trunc(FQ[idx+42],digits=1)
    end
end
...
for itr = 1:last_itr
    ...
    if(iter==1)
        global indicies = findall(arrival_time -> ( arrival_time <= 0),Arrival_Time)
    ...
    #     if(Product_Type[idx] == 6)
    #         total_demand_6 += Max_Order_Quantity[idx]
    #     else
    #         total_demand_9 += Max_Order_Quantity[idx]
    #     end
    #     println("Demand 6 ", total_demand_6," Demand 9 ", total_demand_9 ," order ", idx, " ",  order_Id[idx], " quantity ",Max_Order_Quantity[idx])
    #     # end
    # end

    # total_forecast_demand_6 = 0
    # total_forecast_demand_9 = 0

    # for idx in eachindex(OiD)
    #     if(OiD[idx] > 1100)
    #         if(O[idx] == 6)
    #             total_forecast_demand_6 += ce[idx]
    #         else
    #             total_forecast_demand_9 += ce[idx]
    #         end
    #         println("Forecast 6 ", total_forecast_demand_6, " Forecast 9 " , total_forecast_demand_9 , " order ", OiD[idx], " quantity  ", ce[idx])
    #     end
    # end
    ...
    # forecast_demand_6 = 0
    # forecast_demand_9 = 0
    # if (total_demand_6 < total_forecast_demand_6)
    #     forecast_demand_6 = round((total_forecast_demand_6 - total_demand_6)/14,digits=3)
    # end
    # if (total_demand_9 < total_forecast_demand_9)
    #     forecast_demand_9 = round((total_forecast_demand_9 - total_demand_9)/14,digits=3)
    # end

    # println("forecast demand 6 ", forecast_demand_6, " forecast demand 9 ",  forecast_demand_9)
    # println("total demand 6 ", total_demand_6, " forecast demand 9 ",  total_demand_9)

    # for idx in eachindex(OiD)
    #     if(OiD[idx] > 1100)
    #         if(O[idx] == 6)
    #             ce[idx] = forecast_demand_6
    #             cl[idx] = forecast_demand_6
    #             for fq_idx in eachindex(FQ_ID)
    #                 if(FQ_ID[fq_idx] == OiD[idx])
    #                     FQ[fq_idx] = forecast_demand_6
    #                     FQ_qty[fq_idx] = forecast_demand_6
    #                     break
    #                 end
    #             end

    #         else
    #             ce[idx] = forecast_demand_9
    #             cl[idx] = forecast_demand_9
    #             for fq_idx in eachindex(FQ_ID)
    #                 if(FQ_ID[fq_idx] == OiD[idx])
    #                     FQ[fq_idx] = forecast_demand_9
    #                     FQ_qty[fq_idx] = forecast_demand_9
    #                     break
    #                 end
    #             end
    #         end
    ...
    for idx in indicies
        ...
        if(Response_Time[idx] >= start_time && Latest_Due_Time[idx] >= start_time) # Add only those orders whose response time is not passed
            ...
            #         # println("index in the order list ",index, " forecast: ", ce[index]," Actual: ",Max_Order_Quantity[idx], " Netted: ", netted_qty)
    ...
    for k in eachindex(OiD)
        if(value(Z[k])==1 && OiD[k] in order_Id)
            ...
            Assign_due_time_new[idx] = Int(sum((value(D[k,t]) > 0.5 ? 1 : 0 )*t for t in de[k]:dl[k]))
    ...
    global Fixed_Horizon = min(Fixed_Horizon + Batching_Interval,end_time)
    ...
    for k in eachindex(OiD)
            if(al[k] <= start_time)
                ...
                due_time[k] = Int(sum((value(D[k,t]) > 0.5 ? 1 : 0 )*t for t in de[k]:dl[k]))
                # accepted_quantities[k] = min(cl[k],round(sum(value(C[k,t]) for t in de[k]:dl[k]),digits=3))
                accepted_quantities[k] = min(cl[k],sum(value(C[k,t]) for t in de[k]:dl[k]))
                # accepted_quantities[k] = sum(value(C[k,t]) for t in de[k]:dl[k])
            end
            if(OiD[k] in order_Id)
                global idx = findfirst(id->id==OiD[k],order_Id)
                order_decision[idx] = value(Z[k]) > 0.5 ? 1 : 0
                # Order_promised_quantites[idx] = min(cl[k],round(sum(value(C[k,t]) for t in de[k]:dl[k]),digits=3))
                Order_promised_quantites[idx] = min(cl[k],sum(value(C[k,t]) for t in de[k]:dl[k]))
                # Order_promised_quantites[idx] = sum(value(C[k,t]) for t in de[k]:dl[k])
                order_assigned_due_time[idx] =  Int(sum((value(D[k,t]) > 0.5 ? 1 : 0 )*t for t in de[k]:dl[k]))
            end      
    end
    ...
    global indicies = findall(k -> sum(value(D[k,t])*t for t in de[k]:dl[k]) <= start_time &&  value.(Z[k]) > 0.5, 1:length(al))
    
    for k in indicies
        # update product inventory from the forecasted demand
        if(OiD[k] in FQ_ID)
            # println("#######################################", OiD[k], " ",O[k], " ", sum(value(C[k,t]) for t in de[k]:dl[k]))
            # INV_initial[O[k]] += min(cl[k],round(sum(value(C[k,t]) for t in de[k]:dl[k]),digits=3))
            INV_initial[O[k]] += min(cl[k],sum(value(C[k,t]) for t in de[k]:dl[k]))
        end
    ...
    # remove orders with response time over and were rejected
    indicies = [];
    for k in eachindex(OiD)
        if(al[k] <= start_time)
            global idx = findfirst(id->id==OiD[k],order_Id)
            if (order_decision[idx]<0.5)
                push!(indicies,k)
                println("Removing - Rejected oders and response time over: ",OiD[k])
            end
        end
    end
    deleteat!(OiD,indicies)
    ...
    if (start_time >= Last_arrival_time+1) && (flag<=1)
        global flag = flag+1
    end
    if(flag==2)
        include("ATP_final_decisions_1.jl")
        include("ATP_cases_1.jl")
        global flag = flag+1
    end
